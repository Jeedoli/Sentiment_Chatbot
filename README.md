...
부정 감정이 심한 경우엔 자동으로 상담원 연결을 안내합니다.
...
"배송이 언제 와요?"와 "배송이 또 늦어요?? 진짜 열받아 죽겠네"는  
같은 키워드로 처리되어 동일한 답변이 나오죠.

SentiChat은 두 문장을 다르게 처리합니다:
- 첫 문장 → 중립 감정 → FAQ 안내
- 두 번째 문장 → 부정 감정(확률 87%) → 공감 먼저, 해결책 후, 상담원 연결 안내
...
| **세션 관리** | session_id 기반 멀티 세션, 최근 10턴 히스토리 유지 |
...
헷갈릴 수 있어서 아키텍처 보기 전에 먼저 짚고 갑니다.

```
[ 모델 1 ]  klue/roberta-base (우리가 직접 fine-tuning한 모델)
┌──────────────────────────────────────────────────────────────┐
...
그리고 "klue/roberta-base를 fine-tuning했다"는 말의 실제 의미:
...
"모델을 만들었다"는 신경망을 처음부터 설계한 게 아닙니다.  
한국어를 이미 이해하는 대형 모델에 **3-class 분류 레이어만 추가해서**  
우리 데이터(감정 레이블)로 조정한 것입니다.
...
AI Hub 한국어 감정 데이터셋 기반, 최종 test set 결과입니다.
...
오분류 1건: "환불 방법 알려주세요" → 부정으로 예측 (학습 데이터 특성상 합리적 실수)

---

## 학습 파이프라인에서 공들인 부분

처음엔 120개짜리 합성 데이터로 학습했는데 결과가 처참했습니다.  
"배송 언제 와요?"를 긍정으로 분류할 정도였죠.

문제를 해결하기 위해 세 가지를 바꿨습니다:

**1. 데이터 교체 — AI Hub 실데이터 183K 활용**  
직접 JSON을 파싱해 `build_dataset.py`를 짰습니다.  
25개의 ZIP 파일에서 `RawText`와 `GeneralPolarity`를 추출하고,  
원래 부정:중립:긍정 비율이 13:18:64로 심하게 불균형했던 걸  
언더샘플링으로 1:1:1로 맞췄습니다.
