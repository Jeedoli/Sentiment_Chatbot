...
각 셀의 숫자는 **실제 레이블 → 예측 레이블**로 분류된 샘플 수입니다.  
괄호 안 퍼센트는 해당 실제 클래스 내 비율입니다.

**읽는 방법:**
- 대각선(좌상→우하)이 **정답**입니다. 숫자가 클수록 잘 맞춘 것.
- 대각선 바깥이 **오분류**입니다.

**눈에 띄는 패턴:**
- **부정 → 중립 오분류**가 비교적 적습니다 (91.1% 정확).  
  "진짜 짜증나" 같은 명확한 부정 표현은 잘 잡힙니다.
- **중립의 오분류율이 가장 높습니다** (F1 0.8156).  
  "환불 방법 알려주세요"처럼 요청 문장이 부정/긍정 경계에 걸리기 때문입니다.  
  중립은 정의 자체가 모호해서 사람도 헷갈리는 케이스가 많습니다.
- **긍정 → 중립 오분류**가 일부 발생합니다.  
  "빠른 배송 감사합니다" 같은 차분한 긍정 표현이 중립으로 분류되는 케이스입니다.
...
**Loss 그래프 (왼쪽):**
- Train Loss는 에폭마다 꾸준히 감소합니다.
- Val Loss는 epoch 4 이후 반등합니다. 이게 **과적합 시작 지점**입니다.
- epoch 4에서 Val F1이 최고치(0.8753)를 찍고 이후 하락 → epoch 7에서 Early Stopping 발동.
- `saved_models/sentiment_best.pt`는 epoch 4 시점의 가중치입니다.

**F1 / Accuracy 그래프 (오른쪽):**
- Val F1과 Val Accuracy가 거의 동행합니다. 클래스 균형이 잘 맞춰졌다는 의미입니다.
- epoch 4 이후 성능이 조금씩 떨어지지만 급격히 무너지지는 않습니다.  
  Early Stopping 덕분에 최적 지점에서 멈췄습니다.
